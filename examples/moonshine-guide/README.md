...
[the published guide](https://enjalot.github.io/moonshine/moonshine-guide/).
The article explains the still authoring loop through working simulations of
in-place prose editing, figure knobs, block reordering, linked terms, baked
diagram layouts, shared git history, and routed feedback.

The prose lives in `content/guide.md`; article-specific simulations live under
`src/figures/sim/`. The surrounding Vite + React + Velite project is a full
still scaffold, including the dev-only editing and feedback loop. See the
plugin's [`STILL.md`](../../plugins/moonshine/STILL.md) for its architecture.
...
- **Hold Cmd/Ctrl** to reveal block boundaries; **Cmd/Ctrl+click a
  ...
  raw markdown in place. **Cmd/Ctrl+Enter** or the **✓** button saves; **Esc**
...
src/
  ...
  figures/
    ...
    StillPipelineDiagram.tsx ← the guide's live, baked pipeline diagram
    sim/                   ← working simulations of still authoring features
...
The :term[knob panel]{to=knobs} writes values back to markdown.
...
on `DiagramFigure` (see `StillPipelineDiagram.tsx`) — never Mermaid.

## Single article vs series

This project contains one article, so the root URL renders it directly. The
still scaffold also supports a series: more than one markdown article under
`content/` switches the root to a cards index.
